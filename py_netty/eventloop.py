...
class EventLoop:
    ...
    def __init__(self, pool: threading.Thread):
        assert pool, "thread pool is required"
        ...
        self._epoll.register(self.eventfd, select.POLLIN)
        self._flags = {}
        ...
        self._server_socket = {}  # if the socket is a server socket
    ...
    # def add_pending(self, fileno, pending: bytes):
    #     self._pendings[fileno] = self._pendings.get(fileno, b'') + pending
    #     if self._pendings[fileno]:
    #         self.add_flag(fileno, select.POLLOUT)
    #     else:
    #         del self._pendings[fileno]
    ...
    def write(self, fileno, buffer) -> Future:
        ...
        if self._pendings.get(fileno):  # already has pending
            ...
            pending = sendall(self.connections[fileno], buffer)
    ...
    def _events_to_str(self, events):
        ...
        for fileno, flag in events:
            flags = []
            if fileno == self.eventfd.fileno():
                fdname = "eventfd"
            elif self._server_socket.get(fileno, False):
                fdname = f"server({fileno})"
            elif fileno in self.connections:
                fdname = sockinfo(self.connections[fileno])
            else:
                fdname = "unknown"
    ...
    def _show_debug_info(self):
        ...
        logger.debug("[DEBUG INFO] Connections: %s", self.connections)
        logger.debug("[DEBUG INFO] ServerSockets: %s", self._server_socket)
        logger.debug("[DEBUG INFO] Handlers: %s", self._handlers)

        logger.debug("[DEBUG INFO] Total conns: %s", len(self.connections))
        ...
        logger.debug("[DEBUG INFO] Total accepted: %s", self._accepted)
    ...
    def _start(self):
        ...
        while True:
            ...
            for fileno, event in events:
                if fileno == self.eventfd.fileno():  # just to wake up from epoll
                    # logger.debug("Interrupted")
                    self.eventfd.unsafe_read()
                    continue

                if self._server_socket.get(fileno):
                    for connection, address in acceptall(self.connections[fileno]):
                        self._accepted += 1
                        # print("Total accepted: %s" % self._accepted)
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug("Accepted: %s, address: %s, total: %s", sockinfo(connection), address, self._accepted)
                ...
                if event & select.POLLOUT:
                    ...
                    while True:
                        ...
                        if head.close:  # denote to close locally
                            logger.debug("Close ON_COMPLETE: %s", sockinfo(self.connections[fileno]))
                            self.close_forcibly(fileno, head.future)
                            break
                        l0 = len(head.buffer)
                        head.buffer = sendall(self.connections[fileno], head.buffer)
                ...
                if event & select.POLLIN:
                    buffer = recvall(self.connections[fileno])
